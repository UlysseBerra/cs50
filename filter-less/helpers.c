...
{
    RGBTRIPLE tmp[height][width]; //array for storing a tmporary copy
    ...
    for (int i = 1; i < width - 1; i++)
    {
        image[height][i].rgbtBlue = (int) round((tmp[height][i - 1].rgbtBlue + tmp[height][i].rgbtBlue + tmp[height][i + 1].rgbtBlue + tmp[height - 1][i - 1].rgbtBlue + tmp[height - 1][i].rgbtBlue + tmp[height - 1][i + 1].rgbtBlue) / 6.0);
        image[height][i].rgbtGreen = (int) round((tmp[height][i - 1].rgbtGreen + tmp[height][i].rgbtGreen + tmp[height][i + 1].rgbtGreen + tmp[height - 1][i - 1].rgbtGreen + tmp[height -1][i].rgbtGreen + tmp[height - 1][i + 1].rgbtGreen) / 6.0);
        image[height][i].rgbtRed = (int) round((tmp[height][i - 1].rgbtRed + tmp[height][i].rgbtRed + tmp[height][i + 1].rgbtRed + tmp[height - 1][i - 1].rgbtRed + tmp[height - 1][i].rgbtRed + tmp[height - 1][i + 1].rgbtRed) / 6.0);
    }
    ...
    for (int i = 1; i < height - 1; i ++)
    {
        image[i][width].rgbtBlue = (int) round((tmp[i - 1][width].rgbtBlue + tmp[i][width].rgbtBlue + tmp[i + 1][width].rgbtBlue + tmp[i - 1][width - 1].rgbtBlue + tmp[i][width - 1].rgbtBlue + tmp[i + 1][width - 1].rgbtBlue) / 6.0);
        image[i][width].rgbtGreen = (int) round((tmp[i - 1][width].rgbtGreen + tmp[i][width].rgbtGreen + tmp[i + 1][width].rgbtGreen + tmp[i - 1][width - 1].rgbtGreen + tmp[i][width - 1].rgbtGreen + tmp[i + 1][width - 1].rgbtGreen) / 6.0);
        image[i][width].rgbtRed = (int) round((tmp[i - 1][width].rgbtRed + tmp[i][width].rgbtRed + tmp[i + 1][width].rgbtRed + tmp[i - 1][width - 1].rgbtRed + tmp[i][width - 1].rgbtRed + tmp[i + 1][width - 1].rgbtRed) / 6.0);
    }
