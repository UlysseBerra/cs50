...
void blur(int height, int width, RGBTRIPLE image[height][width])
{
    ...
    // top right corner
    image[0][0].rgbtRed = (int)round((tmp[0][0].rgbtRed + tmp[1][0].rgbtRed + tmp[0][1].rgbtRed + tmp[1][1].rgbtRed) / 4);
    image[0][0].rgbtGreen = (int)round((tmp[0][0].rgbtGreen + tmp[1][0].rgbtGreen + tmp[0][1].rgbtGreen + tmp[1][1].rgbtGreen) / 4);
    image[0][0].rgbtBlue = (int)round((tmp[0][0].rgbtBlue + tmp[1][0].rgbtBlue + tmp[0][1].rgbtBlue + tmp[1][1].rgbtBlue) / 4);

    // bottom right corner
    image[height][0].rgbtRed = (int)round((tmp[height][0].rgbtRed + tmp[height - 1][0].rgbtRed + tmp[height][1].rgbtRed + tmp[height - 1][1].rgbtRed) / 4);
    image[height][0].rgbtGreen = (int)round((tmp[height][0].rgbtGreen + tmp[height - 1][0].rgbtGreen + tmp[height][1].rgbtGreen + tmp[height - 1][1].rgbtGreen) / 4);
    image[height][0].rgbtBlue = (int)round((tmp[height][0].rgbtBlue + tmp[height - 1][0].rgbtBlue + tmp[height][1].rgbtBlue + tmp[height - 1][1].rgbtBlue) / 4);

    // top left corner
    image[0][width].rgbtRed = (int)round((tmp[0][width].rgbtRed + tmp[1][width].rgbtRed + tmp[0][width - 1].rgbtRed + tmp[1][width - 1].rgbtRed) / 4);
    image[0][width].rgbtGreen = (int)round((tmp[0][width].rgbtGreen + tmp[1][width].rgbtGreen + tmp[0][width - 1].rgbtGreen + tmp[1][width - 1].rgbtGreen) / 4);
    image[0][width].rgbtBlue = (int)round((tmp[0][width].rgbtBlue + tmp[1][width].rgbtBlue + tmp[0][width - 1].rgbtBlue + tmp[1][width - 1].rgbtBlue) / 4);

    // bottom left corner
    image[height][width].rgbtRed = (int)round((tmp[height][width].rgbtRed + tmp[height - 1][width].rgbtRed + tmp[height][width - 1].rgbtRed + tmp[height - 1][width - 1].rgbtRed) / 4);
    image[height][width].rgbtGreen = (int)round((tmp[height][width].rgbtGreen + tmp[height - 1][width].rgbtGreen + tmp[height][width - 1].rgbtGreen + tmp[height - 1][width - 1].rgbtGreen) / 4);
    image[height][width].rgbtBlue = (int)round((tmp[height][width].rgbtBlue + tmp[height - 1][width].rgbtBlue + tmp[height][width - 1].rgbtBlue + tmp[height - 1][width - 1].rgbtBlue) / 4);

    // first row
    for (int i = 1; i < width - 1; i++)
    {
        image[0][i].rgbtRed = (int)round((tmp[0][i - 1].rgbtRed + tmp[0][i].rgbtRed + tmp[0][i + 1].rgbtRed + tmp[1][i - 1].rgbtRed + tmp[1][i].rgbtRed + tmp[1][i + 1].rgbtRed) / 6);
    ...
    for (int i = 1; i < height - 1; i++)
    {
        ...
        {
            image[i][j].rgbtRed = (int)(round)((tmp[i - 1][j - 1].rgbtRed + tmp[i - 1][j].rgbtRed + tmp[i - 1][j + 1].rgbtRed + tmp[i][j - 1].rgbtRed + tmp[i][j].rgbtRed + tmp[i][j + 1].rgbtRed + tmp[i + 1][j - 1].rgbtRed + tmp[i + 1][j].rgbtRed + tmp[i + 1][j + 1].rgbtRed) / 9);
